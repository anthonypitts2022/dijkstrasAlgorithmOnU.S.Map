...
public class Dijkstra {
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
        double distanceInX = ux-vx;
        double distanceInY = uy-vy;
        double EuclideanDistance = Math.sqrt(Math.pow(distanceInY,2) + Math.pow(distanceInX,2));
  ...
  public void computeAllEuclideanDistances() {
      ...
      for(Vertex indVertex : allVertices){
          for(Edge indEdge : indVertex.adjacentEdges){
              indEdge.distance = computeEuclideanDistance(indEdge.source.x,indEdge.source.y,indEdge.target.x,indEdge.target.y);
          }
      }
  }
   ...
   */
      public void doDijkstra(String s) {
          ...
          //sets all vertices distances to infinity and known value to false --resetting map
          for(Vertex eachVertex : allVertices){
              eachVertex.distance = Double.POSITIVE_INFINITY;
              eachVertex.known = false;
              eachVertex.prev=null;
          }
          //records initial vertex (city)
          Vertex initPosition = getVertex(s);

          initPosition.distance = 0;
          visited.add(initPosition);
          //loops over all verticies 
          while(visited.isEmpty()==false){
              //pulls first in visited list as currentVertex
              Vertex currentVertex = visited.poll();
              //updates known status of currentVertex
              currentVertex.known=true;
              //adds updated distances of all adjacent edges to currentVertex
              for(Edge indAdjEdge : currentVertex.adjacentEdges){
                  Vertex target = indAdjEdge.target;
                  if(target.known==false){
                      Double prevTargetDistance = indAdjEdge.distance;
                      //if the new route is shorter than current distance to vertex
                      if(target.distance > (currentVertex.distance + prevTargetDistance)){
                          target.distance = currentVertex.distance + prevTargetDistance;
                          //.prev allows for reversal back when reporting best route
                          target.prev = currentVertex;
                          visited.add(target);
                      }
                  }
              }
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
      doDijkstra(s);
      LinkedList<Edge> shortestRoute = new LinkedList<>();
      //gets vertex of destination city (vertex)
      Vertex thisVert = getVertex(t);
      //iterate backwards from destination vertex to source vertex
      for(; false==(thisVert.prev==null); thisVert=thisVert.prev){
          Edge addedEdge = new Edge(thisVert.prev, thisVert, computeEuclideanDistance(thisVert.prev.x,thisVert.prev.y,thisVert.x,thisVert.y));
          //adds addedEdge to head of linked list
          shortestRoute.addFirst(addedEdge);
      }
      //returns a list of edges that are shortest path to destination vertex
